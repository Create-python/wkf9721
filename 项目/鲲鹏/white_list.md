...
class WhiteList:
    ...
    Usage: whitelist scanning
    """
    # whitelist level: 0, 1, 2, 3, 4, 5, 6
    MIGRATION_LEVEL = 5
    TOTAL_LEVEL = 7
    # 不显示的级别为7
    NO_DISPLAYED_LEVEL = '7'
    ...
    def match_whitelist(self, lib_dict, os_name, os_version, file_type=False):
        ...
        :param os_name:
        :param os_version:
        :param file_type: 扫描结果是否添加文件类型，二进制扫描需要，重打包不需要
        :return:
        scan_report[
        ...
        for key in lib_dict.keys():
            ...
            level_number = self.init_level_number()
            scan_report = []
            if so_list:
    ...
    def init_level_number(self):
        """
        初始化level_number
        :return:
        """
        return [0 for _ in range(self.TOTAL_LEVEL)]
    ...
    def scan_result_parameter_check(scan_result, parameter_type):
        ...
        :param parameter_type: scan_result类型
        :return: True/False
        """
        if not scan_result:
    ...
    def level_process(level_number):
        """
        扫描结果等级统计列表处理
        :param level_number: 扫描结果等级统计列表
        :return: 处理后的扫描结果等级统计列表
        """
    ...
    def add_file_type(file_type_flag, match_result, file_type):
        """
        为匹配结果添加文件类型
        :param file_type_flag: 是否添加文件类型标志
    ...
    def _separate_list(scan_report_list):
        """
        将扫描结果细分为动态库列表、二进制文件列表、静态库列表、jar包列表
        :param scan_report_list: 扫描结果
        :return: 动态库列表、二进制文件列表、静态库列表、jar包列表
        """
        exec_file_list = []
        lib_list = []
        jar_list = []
    ...
    class SoMatch:
        ...
        def match_result(self, so_list, os_info, level_number):
            ...
            for lib_list_item in so_list:
                ...
                if lib_temp[-2] != WhiteList.NO_DISPLAYED_LEVEL:
                    level_number[int(lib_temp[-2])] += 1
                    lib_temp = self._deal_scan_result_item(lib_temp)
        ...
        def _deal_scan_result_item(self, scan_result_item):
            """
            处理扫描结果词条，添加文件类型
            :param scan_result_item:
            :return:
            """
            so_name = scan_result_item[0]
            so_version = scan_result_item[1]
            level = scan_result_item[-2]
            if level in self._SO_AND_EXEC_LEVEL:
                if self._judge_head_file_type(so_name):
                    result_type = BinaryScanResultType.DYNAMIC_LIBRARY
                else:
                    result_type = BinaryScanResultType.EXEC
            elif level in self._SOFTWARE_LEVEL:
                result_type = BinaryScanResultType.SOFTWARE
            else:
                if self._judge_head_file_type(so_name):
                    result_type = BinaryScanResultType.DYNAMIC_LIBRARY
                else:
                    if so_version:
                        ...
                        result_type = BinaryScanResultType.EXEC
            scan_result_item = WhiteList.add_file_type(
                self._file_type, scan_result_item, result_type)
            return scan_result_item

        @staticmethod
        def _judge_head_file_type(file_name):
            """
            判断rpm、deb包头部依赖类型
            :param file_name: 文件名
            :return:
            """
            return file_name.endswith('.so') or file_name.find('.so.') > 0

    class JarMatch(SoMatch):
        """ jar匹配内嵌类 """
        def match_result(self, jar_list, os_info, level_number):
            """
            jar匹配结果
            :param jar_list: jar列表
            :param os_info: os信息
            :param level_number: 等级列表
            :return:
            """
            ...
            for jar_item in jar_list:
                ...
                if not self._so_whitelist_dict:
                    self._so_whitelist_dict, _ = \
                        self._generate_so_whitelist_dict(so_list=jar_item[2])
                jar_item_list = []
                for lib_list_item in jar_item[2]:
                    lib_name = os.path.split(lib_list_item)[-1]
                    so_version = 'd'
                    lib_temp_tuple = (lib_name, so_version)
                    if lib_temp_tuple in self._so_whitelist_dict:
                        lib_level_temp = self._so_whitelist_dict.get(
                            lib_temp_tuple)
                    else:
                        lib_level_temp = [str(WhiteList.MIGRATION_LEVEL), '']
                    if lib_level_temp[0] != WhiteList.NO_DISPLAYED_LEVEL:
                        jar_item_list.append(lib_list_item)
                jar_temp = [jar_item[0], jar_version, jar_item[1],
                            jar_item_list] + os_info + level_temp
    ...
    class ExecMatch(SoMatch):
        ...
        def match_result(self, exec_file_list, os_info, level_number):
            ...
            if not self._so_whitelist_dict:
                self._so_whitelist_dict, _ = \
                    self._generate_so_whitelist_dict(exec_file_list)
            for exec_list_item in exec_file_list:
                exec_temp = [exec_list_item[0], exec_version,
                             exec_list_item[1], os_info[0], os_info[1]]
                exec_item = (exec_list_item[0], exec_version)
                lib_temp = [str(WhiteList.MIGRATION_LEVEL), exec_url]
                if exec_item in self._so_whitelist_dict:
                    lib_temp = self._so_whitelist_dict.get(exec_item)
                exec_temp += lib_temp
                exec_temp = WhiteList.add_file_type(
                    self._file_type, exec_temp, BinaryScanResultType.EXEC)
                if lib_temp[0] != WhiteList.NO_DISPLAYED_LEVEL:
                    scan_report_list.append(exec_temp)
                    level_number[int(lib_temp[0])] += 1
            return scan_report_list
    ...
    class StaticMatch:
        ...
        def match_result(self, static_library_list, os_info, level_number):
            """
            静态库文件匹配结果
            :param static_library_list: 静态库文件
            :param os_info: os信息
            :param level_number: 等级列表
            :return:
            """
